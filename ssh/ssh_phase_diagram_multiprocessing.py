import ssh_deformation_dirichlet as sdefd
    ...
    def run(self):
        ...
        while True:
            ...
            if not self.workQueue.empty():
                t1, t2 = self.workQueue.get()
                logger.info(f"{self.name}, acquired parameters {t1}, {t2}")
                self.queueLock.release()
                deformation = sdefd.ssh_deformation_dirichlet(self.k_mesh, param_list=[t1, t2, [0]])
                deformation.find_deformation(300)
...
def get_diagram_list(t1_a, t2_a, k_mesh, num_process=4):
    """ Uses the class diagram_mp to get a list of persistence diagram.
    Args:
        t1_a, t2_a (np.array): array containing the parameters in the Hamiltonian to be used for drawing the phase diagram
    ...
    """
    queueLock = mp.Lock()
    workQueue = mp.Queue(len(t1_a) * len(t2_a))
...
def plot_phase_diagram(param_list, k_means_label, file_name, plot_boundary=True):
    ...
    if plot_boundary == True:
        ax.plot(np.zeros(50) - 1, np.linspace(0.5, 1.5, 50), '-.k')
        ax.plot(np.zeros(50) + 1, np.linspace(0.5, 1.5, 50), '-.k')
...
if __name__ == '__main__':
    num_process = 4
    t1_a = np.linspace(-2.5, 2.5, 50) + np.pi / np.sqrt(6543.21)
    t2_a = np.linspace(1., 1., 1)
    time1 = datetime.datetime.now()
    diagram_list = get_diagram_list(t1_a, t2_a, 100, num_process)
    ...
    save_img(eigenval, eigenvec, k_means_clustered[1], 'figures_ssh/L_rw_spectrum_dir')
    plot_phase_diagram(param_list, k_means_clustered[1], 'figures_ssh/ssh_phase_diagram_dir')
