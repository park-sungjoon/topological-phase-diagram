...
class tb_manifold():
    def __init__(self, parameter_list=None, state_list=None):
        """ Class to be used for isomap or mds. 
        We can either give the list of parameters to be used for tb Hamiltonian ,
        or provide the states through state_list
        Args:
            parameter_list (list): list of the form [t3, t4, idx_list, mesh_size]
        """
    ...
    def get_distance_matrix(self, state_list):
        ...
        for i in range(len(state_list) - 1):
            for j in range(i + 1, len(state_list)):
                distance_matrix[i, j] = distance_matrix[j, i] = tb_utils.distance(state_list[i], state_list[j])
        return distance_matrix

    def get_isomap(self, n_neighbors=5, n_components=2):
        tb_isomap = Isomap(n_neighbors=n_neighbors, n_components=n_components, metric='precomputed')
        isomap_out = tb_isomap.fit_transform(self.distance_matrix)
        return isomap_out, tb_isomap.reconstruction_error()

    def save_isomap(self, file_name, n_neighbors=5, n_components=2):
        assert n_components > 1, 'n_components must be integer greater than 1'
        if n_components == 2:
    ...
    def get_mds(self, n_components=3, n_init=10):
        tb_mds = MDS(n_components=n_components, n_init=n_init, dissimilarity='precomputed')
        tb_mds.fit(self.distance_matrix)
        mds_out = tb_mds.embedding_
        return mds_out, tb_mds.stress_
    ...
    def get_isomap_dim_list(self, n_neighbors=5, max_components=5):
        error_list = []
    ...
    def get_mds_dim_list(self, n_init=10, max_components=5):
        error_list = []
        for n_components in range(1, max_components + 1):
            tb_mds = MDS(n_components=n_components, n_init=n_init, dissimilarity='precomputed')
            tb_mds.fit(self.distance_matrix)
            error_list.append(tb_mds.stress_)
        return error_list


if __name__ == "__main__":
    print('running tb_manifold.py')
    topological = tb_manifold([1.0, 1.0, [0], 200])
    topological.save_isomap('figures_tb/isomap_topological', n_components=3)
    topological.save_mds('figures_tb/mds_topological', n_components=3)
    trivial = tb_manifold([1., -1.0, [0], 200])
    trivial.save_isomap('figures_tb/isomap_trivial', n_components=3)
    trivial.save_mds('figures_tb/mds_trivial', n_components=3)
